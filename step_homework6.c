...
void *my_malloc(size_t size)
{
  ...
  /*metadataにヒープ領域のヘッドを持ってくる
  これを更新して、引数でもらったサイズのオブジェクトを入れる場所を決める
  metadataは容量の前にある | metadata | object |に入っている情報（ポインタではない）
  */
  simple_metadata_t *metadata = simple_heap.free_head;

  //prevには後でmetadataの更新中に更新前のmetadataを保持するのに用いる
  simple_metadata_t *prev = NULL;

  /* First-fit: Find the first free slot the object fits.
  ここでは、metadataを更新して、メモリの最初から探索して
  必要なサイズより大きい容量の場所をみつけたら探索終了*/
  while (metadata && metadata->size < size)
  {
    prev = metadata;
    metadata = metadata->next;
  ...
  /*bestfitを実装(変えた部分はここだけ)
   今求めたmetadata(これが最初のbest_metadataになる)より容量が小さい、
   かつ、sizeより容量が大きいようなmetadataを探す*/
  simple_metadata_t *best_metadata = metadata;
  simple_metadata_t *best_prev = prev;

  /*もし上記のような場所がない＝現在のメモリに必要なサイズの空き容量がないなら
  新しいメモリを要求*/
  if (!metadata)
  {
    // There was no free slot available. We need to request a new memory region
  ...
  /*while条件に注意。firstfitと違い、metadata->nextがnullの場合は終わらせないと、
    if文での比較の際にmetadata->sizeにアクセスしたときにエラーになる*/
  while (metadata->next)
  {
    prev = metadata;
    metadata = metadata->next;

    if (metadata->size < best_metadata->size && size <= metadata->size)
    {
  ...
  //ここでのmetadataはmetadataの情報があるメモリの最初のアドレスを表すポインタ
  void *ptr = best_metadata + 1;

  size_t remaining_size = best_metadata->size - size;
  best_metadata->size = size;
  // Remove the free slot from the free list.
  //freeでなくなったので、この場所はfree_listから外す
  simple_remove_from_free_list(best_metadata, best_prev);

  /*オブジェクトを入れた跡にも余っているメモリ分（remaining_size）が
  metadataを入れるのに必要なサイズより大きいなら、
  新たなfreeの容量がある場所としてfree_listに登録する*/
  if (remaining_size > sizeof(simple_metadata_t))
  {
    // Create a new metadata for the remaining free slot.
    //
  ...
  //引数で与えられたサイズのオブジェクトが格納できるメモリ上の場所のmetadataのアドレスを返す
  return ptr;
}
